...
MID_COLOR  = (0, 255, 255)    # 연결선 / 중간점
AXIS_COLOR = (0, 255,   0)    # 카메라 중심선 (밝은 초록)
...
class PinholeCamera:
    ...
    def _build_map(self, h: int, w: int):
        ...
        K_scaled[0, 2] *= sx
        K_scaled[1, 2] *= sy
        K_new, _ = cv2.getOptimalNewCameraMatrix(K_scaled, self.D, (w, h), alpha=0)
...
class ConeDetector:
    ...
    def _match_pairs(self, reds, blues):
        """
        최근접 거리 매칭
        - 각 red에서 가장 가까운 blue와 매칭
        - 한 번 매칭된 blue는 재사용 불가 → 교차 매칭 방지
        - 쌍 인덱스는 y좌표 기준 (화면 아래 = 가까운 쌍 = Pair0)
        """
        if not reds or not blues:
            return []

        used_blue = set()
        matched   = []  # (dist, red, blue)
        ...
        for r in reds:
            ...
            for j, b in enumerate(blues):
                if j in used_blue:
                    continue
                dist = math.sqrt((r.cx - b.cx)**2 + (r.cy - b.cy)**2)
                if dist < best_dist:
                    best_dist = dist
                    best_blue = (j, b)
            if best_blue is not None:
                used_blue.add(best_blue[0])
                matched.append((best_dist, r, best_blue[1]))

        # 화면 아래쪽(가까운 쌍)부터 Pair0
        matched.sort(key=lambda x: max(x[1].cy, x[2].cy), reverse=True)

        pairs = []
        for pair_idx, (dist, r, b) in enumerate(matched):
            r.index = pair_idx
            b.index = pair_idx
            mid_x   = (r.cx + b.cx) / 2
            mid_y   = (r.cy + b.cy) / 2
            bearing = self.camera.pixel_to_bearing(mid_x)
            pairs.append(ConePair(pair_idx, r, b, mid_x, mid_y, bearing))
...
def draw_results(frame: np.ndarray, pairs, all_cones, img_w: int) -> np.ndarray:
    ...
    # ④ 쌍 연결선 + 중간점 + Bearing 텍스트 (검정 배경 박스)
    for pair in pairs:
        ...
        tx = mx - tw // 2
        ty = max(my - 20, th + 8)   # 화면 위 잘림 방지
        cv2.rectangle(vis, (tx - 4, ty - th - 4), (tx + tw + 4, ty + 4),
                      (0, 0, 0), -1)
